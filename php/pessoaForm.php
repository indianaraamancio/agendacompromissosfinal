...
<div id="alertas">
		...
		<div class="col-md-8">
		 <select name="idRelacao" class="form-control">
		 	<?php
				...
				$comandoRelacoes="SELECT * FROM relacoes";
				$resultadoRelacoes=mysqli_query($conexao,$comandoRelacoes);
				$linhasRelacoes=mysqli_num_rows($resultadoRelacoes);

				if($linhasRelacoes==0){
					echo "<option value='0'>Nenhuma relação cadastrada</option>";
				}else{
					$todasRelacoes=array();

					while($r = mysqli_fetch_assoc($resultadoRelacoes)){
						array_push($todasRelacoes, $r);
					}
					foreach($todasRelacoes as $r){ ?>
					
					<option value="<?=$r['idRelacao']?>"> <?=$r['descricao']?> </option>				
	...
	<table class="table">
		<tr>
		 ...
		 <?php
			$comando="SELECT p.*, r.descricao FROM pessoas as p INNER JOIN relacoes as r ON 
			p.relacoes_idRelacao = r.idRelacao";
			
			if(isset($_GET['pesquisa']) && $_GET['pesquisa']!=""){
				$pesquisa = $_GET['pesquisa'];
				$comando="SELECT p.*, r.descricao FROM pessoas as p INNER JOIN relacoes as r ON 
				p.relacoes_idRelacao = r.idRelacao WHERE p.nome LIKE '".$pesquisa."%'";
			}
			$resultado= mysqli_query($conexao,$comando);
			$linhas=mysqli_num_rows($resultado);

			if($linhas==0){
				echo "<tr><td>Nenhuma pessoa encontrada</td></tr>";
			}else{
			...
			<td><img src="<?=$p['foto']?>" class="imagensConsulta"></td>
			<td><?=$p['nome']?></td>
			<td><?=$p['sobrenome']?></td>
			<td><?=$p['email']?></td>
			<td><?=$p['descricao']?></td>
